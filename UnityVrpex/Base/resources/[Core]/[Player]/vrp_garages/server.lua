		...
		if user_id then
			...
			if multas >= 10000 then
				...
				elseif vRP.vehicleType(tostring(name)) == "work" then
					...
					local spawnveh,vehid = vCLIENT.spawnVehicle(source,name,1000,1000,100,custom,color,0,0,0,0,0,0,true)
					-- vRP.execute("garagem/up_inroad",{ user_id = parseInt(user_id), vehicle = vehName, in_road = 1})
					if spawnveh then
						vehlist[vehid] = { user_id,name }
						TriggerEvent("setPlateEveryone",identity.registration)
					end
...
-- TRYDELETE
-----------------------------------------------------------------------------------------------------------------------------------------
function src.tryDelete(vehid, vehengine, vehbody, vehfuel, vehname, placa, damageData)
 	local user_id = vRP.getUserId(source)
    ...
    if vehlist[vehid] then
        ...
        if vehicle[1] ~= nil then
			...
			print("Erro: ID retornado através da placa do veículo [",placa_user_id,"]")
        end
