...
var templateTrunkChest = (key, amount, image, name, weight, target) => {
  ...
  <div class="cell" data-item-key="${key}">
    ...
    <div class="row">
      <div class="image" style="background-image: url('https://r2.fivemanage.com/3f14FTnNqMTYPNd23NBRQ/images/${image}.png');"></div>
    </div>
    <div class="row">
...
const updateMochila = () => {
  ...
  $.post("http://vrp_trunkchest/requestMochila", JSON.stringify({}), data => {
    ...
    $('.trunk-workspace > .inventory-title').html(`
      Portamalas <small>(<i><b>${data.peso2.toFixed(2)}kg</b> em uso <b>${(data.maxpeso2 - data.peso2).toFixed(2)}kg </b> livre de <b>${data.maxpeso2.toFixed(2)}kg</b></i> no total)</small>
    `);
