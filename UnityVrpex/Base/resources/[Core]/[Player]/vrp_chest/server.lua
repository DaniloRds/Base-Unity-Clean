...
function src.checkIntPermissions(chestName)
    ...
    if nuser_id then
        TriggerClientEvent("Notify", source, "negado", "Você não consegue acessar o baú com pessoas ao lado.")
    else
        if user_id then
            if not vRP.searchReturn(source, user_id) then
                local bauConfig = getBauByName(chestName)
                if bauConfig then
                    if vRP.hasPermission(user_id, bauConfig.perm) or vRP.hasPermission(user_id, "master.permissao") then
                        return true
                    else
                        TriggerClientEvent("Notify", source, "negado", "Você não tem permissão para isso", 8000)
                    end
                else
                    TriggerClientEvent("Notify", source, "negado", "Baú não encontrado.")
                end
            end
        end
    end
...
function getBauByName(name)
    for _, b in pairs(config.baus) do
        if b.name == name then
            return b
        end
    end
...
function src.openChest(chestName)
    ...
    local user_id = vRP.getUserId(source)
    if user_id then
        local bau = getBauByName(chestName)
        ...
        return hsinventory, myinventory, vRP.getInventoryWeight(user_id), vRP.getInventoryMaxWeight(user_id), vRP.computeItemsWeight(result), bau.weight 
    end
    return false
end
-----------------------------------------------------------------------------------------------------------------------------------------
        ...
        if user_id and not processando[chestName]then
            ...
            if items then
                if parseInt(amount) > 0 then
                    ...
                    local bau = getBauByName(chestName)
                    if bau and new_weight <= bau.weight then
