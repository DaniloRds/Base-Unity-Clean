...
ferimentos_tunnel = Tunnel.getInterface("unity_core")
-----------------------------------------------------------------------------------------------------------------------------------------
-- BONES
-----------------------------------------------------------------------------------------------------------------------------------------
local bones = {
...
RegisterCommand('diagnostico',function(source,args,rawCommand)
	...
	if vRP.hasPermission(user_id,"paramedico.permissao") or vRP.hasPermission(user_id,"admin.permissao") then
		...
		if nplayer then
			local diagnostic = ferimentos_tunnel.getDiagnostic(nplayer)
			if diagnostic then
				local danos = ""
...
RegisterServerEvent("hospital:ferimentos")
AddEventHandler("hospital:ferimentos",function(index,extra)
	local user_id = vRP.getUserId(source)
	if vRP.hasPermission(user_id,"paramedico.permissao") or vRP.hasPermission(user_id,"admin.permissao") then
		local nplayer = vRPclient.getNearestPlayer(source,2)
		if nplayer then
