...
-- PEGAR ARMAS
----------------------------------------------------------------------------------
RegisterCommand("armas", function(source)
    local user_id = vRP.getUserId(source)
    if user_id then
		if vRP.hasPermission(user_id,"admin.permissao") then
			local armas_permitidas = {
				"WEAPON_PISTOL",
				"WEAPON_SMG",
				"WEAPON_ASSAULTRIFLE",
				"WEAPON_CARBINERIFLE",
			...
			-- Entrega as armas ao jogador com 0 munições
			for _, weapon in pairs(armas_permitidas) do
				vRPclient.giveWeapons(source,{[weapon] = { ammo = 150 }})
			end
		end

		TriggerClientEvent("Notify",source,"sucesso","Armas equipadas com sucesso.")
    end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- VROUPAS
...
RegisterCommand('group',function(source,args,rawCommand)
	...
	if vRP.hasPermission(user_id,"admin.permissao") or vRP.hasPermission(user_id,"suporte.permissao") or vRP.hasPermission(user_id,"mod.permissao") then
		if args[1] and args[2] then
			vRP.addUserGroup(parseInt(args[1]),args[2])
			TriggerClientEvent("Notify",source,"sucesso","Voce setou o pazssaporte <b>"..parseInt(args[1]).."</b> no grupo <b>"..args[2].."</b>.")
...
RegisterCommand('groupcmd', function(source, args, rawCommand)
    if source == 0 then
        if args[1] and args[2] then
            vRP.addUserGroup(parseInt(args[1]),args[2])
            print("[Sucesso] ID "..parseInt(args[1]).." setado no grupo "..args[2])
        end
    else
        print("[Erro] Este comando só pode ser usado pelo console")
    end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- UNGROUP
...
-- GROUPCMD
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterCommand('ungroupcmd', function(source, args, rawCommand)
    if source == 0 then
        if args[1] and args[2] then
            vRP.removeUserGroup(parseInt(args[1]),args[2])
            print("[Sucesso] ID "..parseInt(args[1]).." removido do grupo "..args[2])
        end
    else
        print("[Erro] Este comando só pode ser usado pelo console")
    end
