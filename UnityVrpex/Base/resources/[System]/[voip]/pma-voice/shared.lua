...
if GetConvar('voice_useNativeAudio', 'false') == 'true' then
	...
	Cfg.voiceModes = {
		{1.0, "Sussurrando"}, -- Whisper speech distance in gta distance units
		{2.3, "Normal"}, -- Normal speech distance in gta distance units
		{5.0, "Gritando"} -- Shout speech distance in gta distance units
	}
else
	Cfg.voiceModes = {
		{3.0, "Sussurrando"}, -- Whisper speech distance in gta distance units
		{7.0, "Normal"}, -- Normal speech distance in gta distance units
		{15.0, "Gritando"} -- Shout speech distance in gta distance units
	}
end
