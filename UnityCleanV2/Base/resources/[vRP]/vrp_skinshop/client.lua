    ...
    while true do
        ...
        for k, v in pairs(lojaderoupa) do
            ...
            if GetDistanceBetweenCoords(playerCoords.x, playerCoords.y, playerCoords.z, lojaderoupa[k].x, lojaderoupa[k].y, lojaderoupa[k].z, true ) <= 5 and not in_loja then
                tdg = 1
               -- DrawText3D(lojaderoupa[k].x, lojaderoupa[k].y, lojaderoupa[k].z-0.1, "~o~[Loja de Roupas]\n~w~Pressione ~o~[E]~w~ para acessar a loja")
                drawText2D("PRESSIONE [~b~E~w~] PARA ACESSAR A ~b~LOJA DE ROUPAS~w~",7,0.5,0.93,0.55,255,255,255,180)
                DrawMarker(21,lojaderoupa[k].x, lojaderoupa[k].y, lojaderoupa[k].z-0.6,0,0,0,0.0,0,0,0.5,0.5,0.5, 43, 125, 189, 255,50,0,0,100)
            end
