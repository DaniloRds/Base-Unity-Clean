...
Citizen.CreateThread(function()
	...
	local function checkTimeOuts()
		...
		while i <= Queue:GetSize() do
			...
			elseif (data.timeout >= 20) and data.source ~= "debug" and os_time() - data.firstconnect > 5 then
				data.deferrals.done(Config.Language._err)
		...
		while i <= Queue:ConnectingSize() do
			...
			if ((data.timeout >= 20 and lastMsg >= 35000) or data.timeout >= 340) and data.source ~= "debug" and os_time() - data.firstconnect > 5 then
				Queue:RemoveFromQueue(data.source, true)
				Queue:RemoveFromConnecting(data.source, true)
			else
				i = i + 1
