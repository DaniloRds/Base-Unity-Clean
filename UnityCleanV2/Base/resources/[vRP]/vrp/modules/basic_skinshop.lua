	...
	if user_id then
		...
		menudata.onclose = function(player)
			...
			if vRP.tryFullPayment(user_id,price) then
				if price > 0 then
					TriggerClientEvent("Notify",source,"sucesso","Você pagou <b>$"..vRP.format(parseInt(price)).." dólares</b> em roupas e acessórios.")
				end
			else
...
local function build_client_skinshops(source)
	...
	if user_id then
		for k,v in pairs(skinshops) do
			...
			vRPclient._addMarker(source,21,x,y,z-0.6,0.5,0.5,0.4,255,128,0,50,100)
