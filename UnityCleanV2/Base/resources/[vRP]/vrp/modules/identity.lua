...
vRP.prepare("vRP/update_user_reg","UPDATE vrp_user_identities SET registration = @registration WHERE user_id = @user_id")
vRP.prepare("vRP/get_userbyphone","SELECT user_id FROM vrp_user_identities WHERE phone = @phone")
vRP.prepare("vRP/update_user_phone","UPDATE vrp_user_identities SET phone = @phone WHERE user_id = @user_id")
...
function vRP.setUserRegistration(user_id,registration,cbr)
	vRP.execute("vRP/update_user_reg",{ user_id = user_id, registration = registration, })
end
...
function vRP.setUserPhone(user_id,phone,cbr)
	vRP.execute("vRP/update_user_phone",{ user_id = user_id, phone = phone, })
end
