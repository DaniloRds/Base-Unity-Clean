...
dPN = {}
Tunnel.bindInterface("dpn_tattoo",dPN)
Proxy.addInterface("dpn_tattoo",dPN)
dPNclient = Tunnel.getInterface("dpn_tattoo")
...
RegisterServerEvent('dpn_tattoo:setPedClient')
AddEventHandler('dpn_tattoo:setPedClient', function()
    local source = source
    local user_id = vRP.getUserId(source)
    local value = vRP.getUData(parseInt(user_id),"vRP:tattoos")
    local custom = json.decode(value) or {}
...
RegisterServerEvent('dpn_tattoo:setPedServer')
