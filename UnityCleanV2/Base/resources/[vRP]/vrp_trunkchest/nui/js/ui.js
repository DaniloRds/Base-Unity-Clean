...
const updateMochila = () => {
  ...
  $.post("http://vrp_trunkchest/requestMochila", JSON.stringify({}), data => {
    ...
    $('.trunk-workspace > .inventory-title').html(`
      Baú do carro <small>(<i><b>${data.peso2.toFixed(2)}kg</b> em uso - <b>${(data.maxpeso2 - data.peso2).toFixed(2)}kg </b> livre de <b>${data.maxpeso2.toFixed(2)}kg</b></i> no total)</small>
    `);
