...
-----------------------------------------------------------------------------------------------------------------------------------------
	["JACK"] = {
		["enter"] = { -1536.88,130.49,57.37 },
		["exit"] = { -786.7,315.81,187.92 },
		["vault"] = { -794.99,326.24,187.32 }
	},
}
-----------------------------------------------------------------------------------------------------------------------------------------
-- HOUSETIMER
...
Citizen.CreateThread(function()
	while true do
		...
		for k,v in pairs(homes) do
			...
			if distance <= 2.5 and houseTimer <= 0 then
				sleep = 5
				DrawText3Ds(v["enter"][1],v["enter"][2],v["enter"][3] + 0.2, "[~y~E~w~] Para ~y~Interagir~w~")
				if IsControlJustPressed(0,38) then
					if vSERVER.checkPermissions(k) then
						houseTimer = 3
...
Citizen.CreateThread(function()
	while true do
		...
		for k,v in pairs(homes) do
			...
			if distance <= 1.6 and houseTimer <= 0 then
				sleep = 5
...
Citizen.CreateThread(function()
	while true do
		...
		for k,v in pairs(homes) do
			...
			if distance <= 1.6 and houseTimer <= 0 then
				sleep = 5
				DrawText3Ds(v["vault"][1],v["vault"][2],v["vault"][3] + 0.2, "[~y~E~w~] Para ~y~Abrir Baú~w~")
				if IsControlJustPressed(0,38) then
					if vSERVER.checkIntPermissions(k) then
						houseTimer = 3
