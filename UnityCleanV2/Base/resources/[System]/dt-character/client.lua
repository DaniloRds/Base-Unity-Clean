RegisterNetEvent("dt-character:normalSpawn")
RegisterNetEvent("dt-character:characterCreate")
...
AddEventHandler("dt-character:characterCreate",function()
	doStatus = 1
...
AddEventHandler("dt-character:normalSpawn",function(firstspawn)
	if firstspawn then
...
RegisterNUICallback('cDoneSave',function(data,cb)
	...
	if GetEntityModel(PlayerPedId()) == GetHashKey("mp_m_freemode_01") then
		...
		SetPedPropIndex(PlayerPedId(),7,3,0)
	end

	SetEntityCoordsNoOffset(PlayerPedId(),437.63,-625.88,28.71,true,true,true)
	SetEntityHeading(PlayerPedId(),f(88.00))
	...
	TriggerServerEvent("dt-character:finishedCharacter",characterNome,characterSobrenome,characterAge,currentCharacterMode)

	-- back to vrp_spawn
	TriggerCamController(-1)
	EndFade()
	doStatus = 2
	--TriggerServerEvent("vrp_spawn:b2k:updateNewChar", characterNome,characterSobrenome,currentCharacterMode)
end)
