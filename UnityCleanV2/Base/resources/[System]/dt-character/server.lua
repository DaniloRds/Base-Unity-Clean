...
function processSpawnController(source,statusSent,user_id)
	...
	elseif statusSent == 1 or statusSent == 0 then
		userlogin[user_id] = true
		TriggerClientEvent("dt-character:characterCreate",source)
	end
end

RegisterServerEvent("dt-character:finishedCharacter")
AddEventHandler("dt-character:finishedCharacter",function(characterNome,characterSobrenome,characterAge,currentCharacterMode)
	local source = source
...
function doSpawnPlayer(source,user_id,firstspawn)
	TriggerClientEvent("dt-character:normalSpawn",source,firstspawn)
