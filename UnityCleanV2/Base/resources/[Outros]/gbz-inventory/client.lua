...
-----------------------------------------------------------------------------------------------------------------------------------------
cRP = {}
Tunnel.bindInterface("gbz-inventory",cRP)
vSERVER = Tunnel.getInterface("gbz-inventory")
-----------------------------------------------------------------------------------------------------------------------------------------
-- INVCLOSE
-----------------------------------------------------------------------------------------------------------------------------------------
...
-- CLOSEINVENTORY
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("gbz_inventory:Close")
AddEventHandler("gbz_inventory:Close",function(	)
	TransitionFromBlurred(1000)
	SetNuiFocus(false,false)
	SetCursorLocation(0.5,0.5)
	SendNUIMessage({ action = "hideMenu" })
...
Citizen.CreateThread(function()
	...
	while true do
		if IsControlJustPressed(0,243) and not IsPlayerFreeAiming(PlayerId()) and GetEntityHealth(PlayerPedId()) >= 102 and not vRP.isHandcuff() then
			...
			SendNUIMessage({ action = "showMenu", slot = slots, mochila = vSERVER.Mochila(), nome = nome, sobrenome = sobrenome, registro = registro, idade = idade, telefone = telefone, multas = multas, coin = coin, banco = banco, carteira = carteira, job = job, vip = vip,vipDays = vipDays, id = user_id})
