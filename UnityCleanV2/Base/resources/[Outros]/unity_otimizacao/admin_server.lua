...
--[ CONEXÃO ]-----------------------------------------------------------------------------------------------------------------
vRPidd = {}
Tunnel.bindInterface("unity_otimizacao",vRPidd)
Proxy.addInterface("unity_otimizacao",vRPidd)
IDDclient = Tunnel.getInterface("unity_otimizacao")
-----------------------------------------------------------------------------------------------------------------------------------------
-- WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookadmin = ""
...
RegisterCommand('status',function(source,args,rawCommand)
	...
	local advogado = vRP.getUsersByPermission("ticket.permissao")
	...
	TriggerClientEvent("Notify",source,"aviso","<bold><b>Jogadores</b> -> "..onlinePlayers.."<br><b>Administração</b> ->  "..#advogado.."<br><b>Policiais</b> -> "..#policia.."<br><b>Ilegal</b> -> "..#ilegal.."<br><b>Paramédicos</b> -> "..#paramedico.."<br><b>Mecânicos</b> -> "..#mec.."</b></bold>",9000)
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- CAR COLOR
-----------------------------------------------------------------------------------------------------------------------------------------
...
RegisterCommand('kickall',function(source,args,rawCommand)
    ...
    if vRP.hasPermission(user_id,"admin.permissao") or vRP.hasPermission(user_id,"suporte.permissao") or vRP.hasPermission(user_id,"mod.permissao") then
        ...
        for k,v in pairs(users) do
            ...
            if id then
                vRP.kick(id,"[ARIZONA] Você foi vitima do terremoto. (Aguarde 30 segundos para entrar na cidade novamente!)")
            end
        end
    end
end)
...
RegisterCommand('coin',function(source,args,rawCommand)
	...
	if vRP.hasPermission(user_id,"owner.permissao") then
		if args[1] then
			...
			vRPclient.setDiv(source,"completerg",".div_completerg { background-color: rgba(0,0,0,0.60); font-size: 13px; font-family: arial; color: #fff; width: 100px; padding: 20px 20px 5px; top: 7%; right: 10%; position: absolute; border: 1px solid rgba(0,0,0,0.2); letter-spacing: 0.5px; } .local { width: 220px; padding-bottom: 15px; float: left; } .local b { color: #DAA520;}","<div class=\"local\"><b>MOEDAS:</b> " ..parseInt(coin).. " </div>")
...
RegisterCommand('s',function(source,args,rawCommand)
	if args[1] then
		...
		local permission = "mod.permissao"
...
function vRPidd.getPermissao()
	local source = source
	local user_id = vRP.getUserId(source)
	if vRP.hasPermission(user_id,"admin.permissao") or vRP.hasPermission(user_id,"admin.permissao") or vRP.hasPermission(user_id,"admin.permissao") or vRP.hasPermission(user_id,"admin.permissao") then
		return true
	else
		return false
	end
